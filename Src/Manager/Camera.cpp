...
void Camera::ProcessMove(void)
{
	...
	else
	{
		...
		// 左スティックの傾き
		moveDir =
			ins.GetDirectionXZAKey(padState.AKeyLX, padState.AKeyLY);
...
void Camera::RotMouse(bool isLimit)
{
	...
	const float sens = 0.00075f;

	angles_.y += deltaX * sens;

	angles_.x += deltaY * sens;
